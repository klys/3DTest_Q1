...
if (_y < 0) {
    if (ex_audio_get_debug_mode()) {
        show_debug_message('exAudio: Error, could not find sound with name "'+_name+'"');
    }
    _y = -1;
}
...
if (instance_exists(obj_ex_audio)) {
	...
	if (ex_audio_get_debug_mode()) {
        show_debug_message('exAudio: Warning, exAudio system is already initialized');
    }
...
#define ex_audio_smoothstep
///ex_audio_smoothstep(upperBound, lowerBound, t)
...
_p = ((argument[2] - argument[0]) / (argument[1] - argument[0]));
return (_p * _p * (3 - 2 * _p));
...
if (audio_system() == audio_new_system) {
	...
	// paused state not supported in legacy audio system
	return 0;
}
...
if (audio_system() == audio_new_system) {
	...
	return sound_isplaying(_list[# _ex_audio._resource, _y]);
}
...
if (audio_system() == audio_new_system) {
	...
	//show_debug_message(_audio_length);
}
...
if (audio_system() == audio_new_system) {
	...
	_list[# _ex_audio._paused,  _y] = false;
	_list[# _ex_audio._looping, _y] = false;
    sound_stop(_sound);
...
if (_duration > 0) {
	...
	if (audio_system() == audio_new_system) {
		...
		// pitch not available in legacy audio system
		return 0;
	}
...
if (_duration > 0) {
	...
	if (audio_system() == audio_new_system) {
		...
		sound_volume(_list[# _ex_audio._resource, _y], _volume);
	}
